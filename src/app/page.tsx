...
const defaultCommunities: StoredCommunity[] = circlesConfig.defaultRecipientAddress ? [{
  ...
  name: "Commons Lab",
  description: "An Organization treasury for funding local projects with CRC.",
  kind: "organization",
    ...
    <main>
      <header className="border-b border-ink/10 bg-cream/90 px-5 py-4 backdrop-blur md:px-8">
        <nav className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex w-full flex-wrap items-center gap-2 sm:w-auto">{isMiniappHost && <span className={`rounded-full px-2.5 py-2 text-[10px] font-bold uppercase tracking-wider ${isHostWalletConnected ? "bg-moss/10 text-moss" : "bg-coral/10 text-coral"}`}>{isHostWalletConnected ? "Gnosis connected" : "Waiting for Gnosis"}</span>}<button type="button" onClick={() => setShowCommunityPicker(true)} className="flex max-w-36 items-center gap-1.5 rounded-full border border-ink/15 bg-white px-3 py-2 text-left text-xs font-semibold text-ink transition hover:border-indigo/35 sm:max-w-44"><Building2 className="h-3.5 w-3.5 shrink-0 text-indigo" /><span className="truncate">{activeCommunity?.name ?? "Choose treasury"}</span><ChevronDown className="h-3.5 w-3.5 shrink-0 text-ink/40" /></button>{!isMiniappHost && <><Button size="sm" variant="ghost" onClick={() => { setCommunityStep("idle"); setCommunityName(""); setCommunityDescription(""); setCommunityError(""); setCommunityModal("create"); }}><Plus className="h-4 w-4" />New treasury</Button><Button size="sm" variant="outline" onClick={() => { setCommunityError(""); setCommunityModal("manage"); }}><Building2 className="h-4 w-4" />Manage</Button></>}</div>
        </nav>
      </header>
      ...
      <section className="px-5 pb-14 pt-14 md:px-8 md:pb-20 md:pt-20">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <div>
            <p className="mb-5 inline-flex items-center gap-2 rounded-full border border-moss/20 bg-moss/5 px-3 py-1 text-xs font-bold uppercase tracking-[0.18em] text-moss"><Sparkles className="h-3.5 w-3.5" />Organization treasury for local commons</p>
            ...
            <p className="mt-6 max-w-xl text-base leading-7 text-ink/65">Create a Circles Organization treasury, publish concrete local projects, receive CRC contributions, and let the admin pay funds out when work happens.</p>
          ...
          <div className="rounded-[2rem] border border-ink/10 bg-white/75 p-6 shadow-[0_24px_60px_-32px_rgba(37,27,159,0.35)]">
            <div className="flex items-center justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-ink/45">Selected treasury</p><p className="mt-2 font-display text-3xl font-bold tracking-tight">{activeCommunity?.name ?? "Choose treasury"}</p><div className="mt-2 flex flex-wrap gap-2"><p className="w-fit rounded-full bg-indigo/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-indigo">{activeCommunityKindLabel}</p>{activeCommunity?.source === "activated" && <p className="w-fit rounded-full bg-moss/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-moss">Activated on Commons</p>}</div><p className="mt-2 text-xs leading-5 text-ink/50">{activeCommunity?.description ?? "Choose an Organization treasury to see its funded projects."}</p><p className="mt-2 text-[11px] leading-5 text-ink/40">Project contributions go to {shortAddress(communityTreasuryAddress)}. Admin payouts move CRC from this treasury.</p></div><div className="rounded-2xl bg-moss/10 p-3 text-moss"><HandHeart className="h-6 w-6" /></div></div>
            ...
            <div className="mt-6 rounded-2xl bg-sand/65 p-4 text-sm leading-6 text-ink/65">CRC moves from contributors into the Organization treasury, then out to the people making projects happen.</div>
          </div>
        </div>
      </section>
      ...
      <section id="projects" className="px-5 py-14 md:px-8 md:py-20"><div className="mx-auto max-w-6xl"><SectionHeading eyebrow="Funded projects" title={`Projects funded by ${activeCommunity?.name ?? "the selected treasury"}`} description="These are concrete funding proposals owned by the selected Organization treasury." />
        <div className="mt-8 grid gap-5 md:grid-cols-2">{projects.map((project) => <article key={project.id} className="rounded-3xl border border-ink/10 bg-white/80 p-6 shadow-[0_16px_30px_-28px_rgba(15,23,42,0.45)]">
          <div className="flex items-start justify-between gap-4"><div><p className="mb-2 w-fit rounded-full bg-moss/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-moss">Funded by {activeCommunity?.name ?? "treasury"}</p><h3 className="font-display text-2xl font-bold tracking-tight">{project.title}</h3><p className="mt-2 flex items-center gap-1.5 text-xs font-medium text-ink/50"><MapPin className="h-3.5 w-3.5" />{project.location}</p></div><div className="rounded-2xl bg-moss/10 p-3 text-moss"><Leaf className="h-5 w-5" /></div></div>
      ...
      {showCommunityPicker && <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/45 p-0 backdrop-blur-sm sm:items-center sm:p-5"><div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-t-[2rem] bg-cream p-5 shadow-2xl sm:rounded-[2rem] sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo">Treasury directory</p><h2 className="mt-2 font-display text-2xl font-bold tracking-tight">Choose an Organization treasury</h2><p className="mt-2 text-sm leading-6 text-ink/60">Projects, contributions and admin actions change with the selected treasury.</p></div><button type="button" onClick={() => setShowCommunityPicker(false)} className="rounded-full border border-ink/10 bg-white p-2 text-ink/55" aria-label="Close treasury directory"><X className="h-4 w-4" /></button></div>
        <div className="mt-5 space-y-3">{organizationTreasuries.map((community) => { const selected = community.address.toLowerCase() === recipientAddress.toLowerCase(); const kindLabel = "Circles Organization"; return <button key={community.address} type="button" onClick={() => selectCommunity(community.address)} className={`w-full rounded-2xl border p-4 text-left transition ${selected ? "border-indigo/30 bg-indigo/5 shadow-[0_12px_25px_-22px_rgba(37,27,159,0.55)]" : "border-ink/10 bg-white/80 hover:border-indigo/25"}`}><div className="flex items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><p className="font-display text-lg font-bold tracking-tight">{community.name}</p><span className="rounded-full bg-indigo/10 px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider text-indigo">{kindLabel}</span></div><p className="mt-2 text-sm leading-6 text-ink/60">{community.description || "A Circles Organization treasury for local projects."}</p></div>{selected && <span className="rounded-full bg-indigo p-1 text-white"><Check className="h-3.5 w-3.5" /></span>}</div><div className="mt-3 flex flex-wrap items-center gap-2 text-[11px] text-ink/40"><span className="font-mono">{shortAddress(community.address)}</span><span>treasury {shortAddress(community.treasuryAddress ?? community.address)}</span></div></button>; })}</div>
        {!isMiniappHost && <Button variant="outline" className="mt-5 w-full" onClick={() => { setShowCommunityPicker(false); setCommunityStep("idle"); setCommunityName(""); setCommunityDescription(""); setCommunityError(""); setCommunityModal("create"); }}><Plus className="h-4 w-4" />Create Organization treasury</Button>}
      </div></div>}
      ...
      {communityModal && <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/45 p-0 backdrop-blur-sm sm:items-center sm:p-5"><div className="max-h-[95vh] w-full max-w-lg overflow-y-auto rounded-t-[2rem] bg-cream p-5 shadow-2xl sm:rounded-[2rem] sm:p-6">
        <div className="flex items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-indigo">Organization treasury</p><h2 className="mt-2 font-display text-2xl font-bold tracking-tight">{communityModal === "create" ? "Create treasury" : `Manage ${activeCommunity?.name ?? "treasury"}`}</h2></div><button type="button" onClick={() => setCommunityModal(null)} className="rounded-full border border-ink/10 bg-white p-2 text-ink/55" aria-label="Close treasury panel"><X className="h-4 w-4" /></button></div>
        {communityModal === "create" ? communityStep === "created" ? <div className="py-8 text-center"><div className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-moss/10 text-moss"><CheckCircle2 className="h-8 w-8" /></div><h3 className="mt-5 font-display text-2xl font-bold">Organization treasury created</h3><p className="mt-2 text-sm leading-6 text-ink/60">Your Circles Organization is registered and selected. Projects can now receive CRC into this treasury.</p><div className="mt-5 rounded-2xl border border-moss/20 bg-moss/5 p-3 text-left"><p className="text-xs font-bold uppercase tracking-wider text-moss">Organization address</p><p className="mt-2 break-all font-mono text-xs text-ink/70">{organizationAddress}</p></div><Button className="mt-6" onClick={() => { setCommunityStep("idle"); setCommunityModal("manage"); }}>Manage treasury</Button></div> : <>
          <p className="mt-4 text-sm leading-6 text-ink/60">Create a Circles Organization on Gnosis Chain. It works as the shared treasury for local funded projects and requires Rabby or MetaMask with xDAI for gas.</p>
          ...
          <label className="mt-4 block text-xs font-bold uppercase tracking-wider text-ink/50">Treasury name<input value={communityName} onChange={(event) => setCommunityName(event.target.value)} placeholder="e.g. Commons Lab" className="mt-2 w-full rounded-xl border border-ink/10 bg-white px-3 py-2.5 text-sm font-normal normal-case tracking-normal outline-none transition focus:border-indigo/45" /></label>
          <label className="mt-4 block text-xs font-bold uppercase tracking-wider text-ink/50">Description<textarea value={communityDescription} onChange={(event) => setCommunityDescription(event.target.value)} placeholder="What will this treasury fund?" rows={3} className="mt-2 w-full resize-none rounded-xl border border-ink/10 bg-white px-3 py-2.5 text-sm font-normal normal-case tracking-normal outline-none transition focus:border-indigo/45" /></label>
