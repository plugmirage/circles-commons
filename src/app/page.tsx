...
function isProjectComplete(project: Pick<Project, "raised" | "goal" | "status" | "deadline">) {
  return project.status === "completed" || project.status === "withdrawn" || project.raised >= project.goal || isDeadlineExpired(project.deadline);
}
    ...
    <main>
      ...
      <section id="projects" className="px-5 py-14 md:px-8 md:py-20"><div className="mx-auto max-w-6xl"><SectionHeading eyebrow="Funded projects" title="Open projects" description="These are public funding proposals created by Gnosis App wallets. Contributions go into the escrow contract and update from on-chain events." />
        <div className="mt-8 grid gap-5 md:grid-cols-2">{projects.map((project) => {
          ...
          const manuallyCompleted = project.status === "completed";
          ...
          const withdrawable = ownerMatches && !withdrawn && !manuallyCompleted && (goalReached || deadlineEnded);
          ...
          <div className="flex items-start justify-between gap-4"><div><div className="mb-2 flex flex-wrap gap-2"><p className="w-fit rounded-full bg-moss/10 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-moss">Created by {project.ownerAddress ? <a href={`https://gnosisscan.io/address/${project.ownerAddress}`} target="_blank" rel="noreferrer" className="underline decoration-moss/40 underline-offset-2">{creatorName}</a> : "early demo"}</p>{completed && <p className="w-fit rounded-full bg-indigo px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white">{withdrawn ? "Funds withdrawn" : manuallyCompleted ? "Completed" : goalReached ? "Goal reached" : "Deadline ended"}</p>}</div><h3 className="font-display text-2xl font-bold tracking-tight">{project.title}</h3><p className="mt-2 flex items-center gap-1.5 text-xs font-medium text-ink/50"><MapPin className="h-3.5 w-3.5" />{project.location}</p></div><div className="rounded-2xl bg-moss/10 p-3 text-moss"><Leaf className="h-5 w-5" /></div></div>
          <div className="mt-4">
